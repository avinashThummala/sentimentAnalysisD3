...
def search(auth, query, number_of_tweets):
	...
	print 'Searching Twitter to try and find %s Tweets about "%s"' % (number_of_tweets, query)

	#keep getting more data until the number of tweets have been gathered
	while True: 

		print 'Performing a search iteration, found %s Tweets thus far' % len(tweets)
		...
		for status in data['statuses']:
			...
			if text.find('RT ') != 0:
				...
				if counter>number_of_tweets:
					print 'Found all %s Tweets!' % number_of_tweets
					return tweets					
		...
		if 'next_results' in data['search_metadata']:
			...
			print 'Sorry, I could only find %s Tweets instead of %s' % (counter, number_of_tweets)
			return tweets
...
def returnStats(tweets):
	"""
	Calculate and return some basic summary statistics
	...
	return [ data['entity']['positive'], data['entity']['negative'], data['entity']['neutral'] ], [ data['doc']['positive'], data['doc']['negative'], data['doc']['neutral'] ]	

def runAnalyzer(query, count):
	"""
